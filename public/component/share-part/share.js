...
export function toggleShare(e, type) {
  ...
  const id = e && !type ? e.target.getAttribute('share-id') : localStorage.getItem('id')
...
async function loadList() {
  ...
  if (localStorage.getItem('loadShareFriend') || localStorage.getItem('onPage') == '2') {
    return
  }
...
function sendMessage(e) {
  const id = document.querySelector('div[message-id]').getAttribute('message-id');
  const user = document.querySelector(`.friend #name[name-id="${e.target.getAttribute('x-data')}"]`).textContent;
